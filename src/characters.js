...
export function Ghost(x, y, width, height, id) {
  this.x = x;
  this.y = y;
  this.width = width;
  this.height = height;
  ...
  this.draw = function(ctx, game) {
    ...
    if (this.status === "afraid") {
      if (ticks % 2 === 0) image = images.gAfraid0;
      else image = images.gAfraid1;
    } else if (this.status === "dead") {
      if (this.y > game.pacman.y) image = images.gDead0;
      else image = images.gDead1;
    } else {
      image = images.ghostImages[this.id % 4][this.direction][ticks % 2];
    }
...
export function Pacman(x, y, width, height) {
  this.x = x;
  this.y = y;
  ...
  this.draw = function(ctx, game) {
    ...
    var image = images.pacmanImages[this.direction][ticks % 4];
    ctx.drawImage(image, this.x, this.y, this.width, this.height);
  };
