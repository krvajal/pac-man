...
export function Game(
  ...
  for (var i = 0; i < this.R; i++)
    for (var j = 0; j < this.C; j++) {
      ...
      if ("0123456789".indexOf(board[i][j]) !== -1)
        this.ghosts.push(
          new Ghost(x, y, this.cellW, this.cellH, this.board[i][j] % 4)
        );
    }

  this.ghostBaseDistance = BFS(this.baseR, this.baseC, this.board, "#");

  document.onkeydown = function(event) {
    if (37 <= event.which && event.which <= 40) {
      if (event.which === 37) self.pacman.nextDirection = LEFT;
      else if (event.which == 38) self.pacman.nextDirection = UP;
      else if (event.which == 39) self.pacman.nextDirection = RIGHT;
      else if (event.which == 40) self.pacman.nextDirection = DOWN;
  ...
  function handleGesture() {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      if (dx < 0) self.pacman.nextDirection = LEFT;
      else self.pacman.nextDirection = RIGHT;
    } else {
      if (dy < 0) self.pacman.nextDirection = UP;
      else self.pacman.nextDirection = DOWN;
    }
  }
  ...
  this.canMove = function(character, direction) {
    if (character.x % this.cellW === 0 && character.y % this.cellH === 0) {
      var r =
        (this.getRow(character) + DIRECTIONS[direction][0] + self.R) % self.R;
      var c =
        (this.getCol(character) + DIRECTIONS[direction][1] + self.C) % self.C;
      if (board[r][c] !== "#")
      ...
      if (character.status === "chase") {
        ...
        for (var i = 0; i < DIRECTIONS.length; i++)
          if (this.canMove(character, i)) {
            var r =
              (this.getRow(character) + DIRECTIONS[i][0] + self.R) % self.R;
            var c =
              (this.getCol(character) + DIRECTIONS[i][1] + self.C) % self.C;
            if (
              bestDistance === -1 ||
    ...
    this.ghosts.forEach(function(ghost) {
      ...
      if (self.collide(ghost, self.pacman)) {
        if (ghost.isDangerous()) {
          ...
          sounds.pacmanEatghostAudio.play();
        }
      }
    ...
    for (var i = 0; i < this.R; i++)
      for (var j = 0; j < this.C; j++) {
        if (this.board[i][j] === ".") {
          ...
          var y = i * this.cellH;
          var margin = 2;
          ctx.drawImage(images.dotS, x, y, this.cellW, this.cellH);
          dots += 1;
          ...
          if (frame % 4 === 0) {
            ...
            ctx.drawImage(images.dotM, x, y, this.cellW, this.cellH);
          } else if (frame % 4 == 2) {
            ctx.drawImage(images.dotB, x, y, this.cellW, this.cellH);
          } else if (frame % 4 == 3) {
            ctx.drawImage(images.dotM, x, y, this.cellW, this.cellH);
          }
          dots += 1;
  ...
  this.over = function() {
    ...
    sounds.pacmanDeathAudio.play();
    sounds.pacmanDeathAudio.onended = function() {
      self.lives -= 1;
      $($("#lives img")[0]).remove();
      if (self.lives > 0) {
  ...
  this.start = function() {
    ...
    this.draw();
    var promise = sounds.pacmanBeginningAudio.play();
    if (promise !== undefined) {
      promise.then(
        function() {
          sounds.pacmanBeginningAudio.onended = function() {};
        },
        function(error) {
